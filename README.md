...
Stack 是一个大规模编码器-解码器基础模型，在 1.5 亿统一预处理的单细胞上训练。它提出了一种新颖的 **表格注意力（Tabular Attention）** 架构，以"细胞 x 基因"矩阵块作为基本输入单元，同时支持**细胞内**和**细胞间**的信息流动。通过上下文学习（in-context learning），Stack 在泛化生物效应方面表现优异，并能够在全新语境下生成未观测到的细胞表达谱。
